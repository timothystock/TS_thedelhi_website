...
?><table id="addresses" cellspacing="0" cellpadding="0" style="width: 100%; vertical-align: top; margin-bottom: 40px; padding:0;" border="0">
	...
	<?php
		...
		if (!empty($woocommerce_opening_hours->wc_compat)) { 
			$time = $woocommerce_opening_hours->wc_compat->get_meta($order, '_openinghours_time', true);
			$check_choice = $woocommerce_opening_hours->get_customer_choice();
			if ($time && 'noorders' !== $check_choice) {
				?><tr><td valign="top" width="100%" colspan="2"><?php
				echo '<p id="openinghours_adminlabel"><strong>'.apply_filters('openinghours_adminlabel', __('Time chosen', 'openinghours')).":</strong> ".$woocommerce_opening_hours->get_display_time_from_meta($time)."</p>";
				?></td></tr>
			<?php } ?>
		<?php } ?>
</table>
